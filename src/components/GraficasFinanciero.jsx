...
// ── GRÁFICAS INGENIERO ───────────────────────────────────────────────────────
// Paleta unificada con la app (C, en constants.js): colores corporativos
// primero, extendidos con tonos de la misma familia para completar 8 categorías.
const NEON=[C.blue,C.green,C.blueMid,C.yellow,'#e0824f','#7b6bd6','#1a8fa0','#c2447a'];
const CHART_BG=C.bgCard2;
const GC={grid:C.border,axis:C.muted,label:C.text,muted:C.muted,danger:C.danger};
...
const NeonBar=(props)=>{
  ...
  return(
    <rect x={x} y={y} width={width} height={height} fill={fill} rx={3}
      style={{filter:`drop-shadow(0 2px 3px ${fill}66)`}}/>
  );
};
...
export function GraficasFinanciero({financiero, tramites, cumAnterior}){
  ...
  const TT={contentStyle:{backgroundColor:C.bgCard,border:`1px solid ${C.border}`,borderRadius:8,color:GC.label,fontSize:11,boxShadow:'0 2px 8px #0001'},labelStyle:{color:GC.label,fontWeight:700}};
  ...
  return (
    <Card style={{marginBottom:16}}>
      ...
      {finItems.length>0&&<>
        ...
        <div style={{background:CHART_BG,borderRadius:14,padding:'16px 8px 8px',border:`1px solid ${C.border}`}}>
          <ResponsiveContainer width="100%" height={Math.max(140,finItems.length*46)}>
            <BarChart data={barFinData} layout="vertical" margin={{left:4,right:52,top:2,bottom:2}}>
              ...
              <RcBar dataKey="porEjecutar" name="Restante" stackId="s" fill={C.border} radius={[0,4,4,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
        {resumenBar&&(
          <div style={{fontSize:11,color:GC.muted,marginTop:6,padding:'7px 12px',background:C.bgCard2,borderRadius:8,lineHeight:1.7,border:`1px solid ${C.border}`}}>
            📌 Promedio: <b style={{color:C.blue}}>{resumenBar.avg}%</b>
            {' · '}Más avanzado: <b style={{color:C.green}}>{resumenBar.best.name} ({resumenBar.best.pctReal}%)</b>
            {' · '}Más rezagado: <b style={{color:C.danger}}>{resumenBar.worst.name} ({resumenBar.worst.pctReal}%)</b>
          </div>
        )}
        ...
        {radialData.length>1&&<>
          ...
          <div style={{background:CHART_BG,borderRadius:14,padding:'12px 8px 8px',border:`1px solid ${C.border}`}}>
            <ResponsiveContainer width="100%" height={Math.max(180,radialData.length*26+80)}>
              <RadialBarChart cx="50%" cy="50%" innerRadius="18%" outerRadius="92%"
                ...
                <RadialBar dataKey="value" background={{fill:C.border}} cornerRadius={5}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'3px 20px',padding:'4px 12px 8px'}}>
              {radialData.map((d,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:7,fontSize:10,minWidth:0}}>
                  <div style={{width:8,height:8,borderRadius:'50%',background:d.fill,flexShrink:0,boxShadow:`0 1px 3px ${d.fill}88`}}/>
            ...
            <div style={{fontSize:11,color:GC.muted,padding:'6px 12px 4px',borderTop:`1px solid ${C.border}`}}>
              💰 Presupuesto total: <b style={{color:C.blue}}>{fmt(totalPres)}</b>
